<script>
  ...
  export let itemType = "quiz"; // quiz or exam
  ...
  async function handleDelete() {
    ...
    const url = deletionUrl || `/apis/exams/${item?.exam_code}`;
    loading = true;
    try {
      ...
      if (response.status === 204) {
        dispatch("success", {
          item,
          message: `Successfully deleted ${itemType} '${item?.exam_name}'.`,
        });
        return;
      }
      ...
      <div class="flex items-center gap-2">
        ...
        <div>
          <h2 class="text-base font-semibold text-red-600">
            About to delete the {itemType} - {item?.exam_name || "-"}
          </h2>
        </div>
      </div>
    </div>
      ...
      <div class="bg-gray-50 p-4 rounded-lg shadow-sm">
        <dl class="space-y-2">
          <div class="flex items-center">
            ...
            <dd class="text-sm text-dark-gray">{item?.exam_name}</dd>
          </div>
          <div class="flex">
            <dt class="text-sm font-medium text-gray-500 w-24">Code:</dt>
            <dd class="text-sm text-dark-gray">{item?.exam_code}</dd>
          </div>
          <div class="flex">
            <dt class="text-sm font-medium text-gray-500 w-24">Status:</dt>
            <dd class="text-sm text-dark-gray">{item?.status}</dd>
          </div>
        </dl>
      </div>
