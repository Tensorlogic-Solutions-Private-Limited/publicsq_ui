<script>
  ...
  async function handleDelete() {
    ...
    loading = true;
    try {
      const response = await apiClient(`/apis/organizations/${organization.uuid}`, {
        method: "DELETE",
      });
      ...
      if (response.status === 400) {
        throw new Error("Cannot delete organization with existing active blocks");
      }
    ...
    if (!response || !response.ok) {
      ...
      dispatch("success", {
        organization,
        message: `Successfully deleted organization '${organization.org_name}'.`,
      });
    } catch (err) {
      console.error("Error deleting organization:", err);
      error = err.message || "Failed to delete organization. Please try again.";
    } finally {
      loading = false;
    }
  }
    ...
    <div class="flex items-center gap-2">
      ...
      <div class="">
        <h2 class="text-base font-semibold text-red-600">
          About to delete the Organization - {organization?.org_name || "-"}
        </h2>
      </div>
    </div>
  </div>
    ...
    <div class="bg-gray-50 p-4 rounded-lg shadow-sm">
      <dl class="space-y-2">
        <div class="flex items-center">
          ...
          <dd class="text-sm text-dark-gray">{organization.org_code}</dd>
        </div>
        <div class="flex">
          <dt class="text-sm font-medium text-gray-500 w-24">Name:</dt>
          <dd class="text-sm text-dark-gray">{organization.org_name}</dd>
        </div>
        <div class="flex">
          <dt class="text-sm font-medium text-gray-500 w-24">Description:</dt>
          <dd class="text-sm text-dark-gray">{organization.org_description || "N/A"}</dd>
        </div>
        <div class="flex">
          <dt class="text-sm font-medium text-gray-500 w-24">Status:</dt>
          <dd class="text-sm text-dark-gray">{organization.is_active ? "Active" : "Inactive"}</dd>
        </div>
      </dl>
    </div>
