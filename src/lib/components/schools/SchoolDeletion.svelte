<script>
  ...
  async function handleDelete() {
    ...
    loading = true;
    try {
      const response = await apiClient(`/apis/schools/${school.uuid}`, {
        method: "DELETE",
      });
      ...
      dispatch("success", {
        school,
        message: `Successfully deleted school '${school.school_name}'.`,
      });
    } catch (err) {
      console.error("Error deleting school:", err);
      error = err.message || "Failed to delete school. Please try again.";
    } finally {
      loading = false;
    }
  }
    ...
    <div class="flex items-center gap-2">
      ...
      <div class="">
        <h2 class="text-base font-semibold text-red-600">
          About to delete the School - {school?.school_name || "-"}
        </h2>
      </div>
    </div>
  </div>
    ...
    <div class="bg-gray-50 p-4 rounded-lg shadow-sm">
      <dl class="space-y-2">
        <div class="flex items-center">
          ...
          <dd class="text-sm text-dark-gray">{school.school_name}</dd>
        </div>
        <div class="flex">
          <dt class="text-sm font-medium text-gray-500 w-24">Code:</dt>
          <dd class="text-sm text-dark-gray">{school.udise_code}</dd>
        </div>
        <div class="flex">
